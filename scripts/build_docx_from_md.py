...
def force_black_xml(xml: str) -> str:
    ...
    _replace_file_robust(tmp_path, docx_path)

def _replace_file_robust(src: Path, dst: Path, retries: int = 5, delay: float = 0.3) -> None:
    """Robustly replace dst with src, handling Windows file locks with retries."""
    import time
...
def normalize_docx_text_color(docx_path: Path) -> None:
    ...
    with zipfile.ZipFile(docx_path, "r") as src_zip, zipfile.ZipFile(tmp_path, "w", compression=zipfile.ZIP_DEFLATED) as dst:
